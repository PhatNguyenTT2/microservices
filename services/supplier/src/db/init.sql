...
-- ============================================================
-- CLEANUP: Remove duplicate suppliers (keep lowest ID per name)
-- Safe: skips suppliers referenced by purchase_orders
-- ============================================================
DELETE FROM supplier
WHERE id NOT IN (
    -- Keep the first (lowest ID) supplier for each company_name
    SELECT MIN(id) FROM supplier GROUP BY company_name
)
AND id NOT IN (
    -- Protect any supplier referenced by purchase_orders
    SELECT DISTINCT supplier_id FROM purchase_order
);

-- ============================================================
-- SEED DATA: Suppliers (only if table is empty)
-- ============================================================
DO $$ BEGIN
  -- Only seed if table is empty (prevents duplicates on restart)
  IF NOT EXISTS (SELECT 1 FROM supplier LIMIT 1) THEN
