...
CREATE TABLE product_knowledge_base (
    ...
    fts_content TSVECTOR,                    -- Cho Keyword Search (Hybrid)
    category_name TEXT,
...
Từ khóa: coca cola, coca-cola, nước ngọt.
```

Mẫu này được thiết kế **tối ưu cho Hybrid Search** vì:
- **Câu văn tự nhiên** → Vietnamese SBERT nắm bắt ngữ nghĩa đa chiều.
- **"Từ khóa:"** → tsvector bắt chính xác tên sản phẩm, thương hiệu, tên không dấu.
...
sequenceDiagram
    ...
    participant KB as KnowledgeBase (pgvector + tsvector)
    ...
    CS->>RAG: recommend(message, storeId, customerId, chatHistory)

    Note over RAG,QR: Bước 2: Viết lại câu hỏi (nếu mơ hồ)
    RAG->>QR: reformulate(message, chatHistory)
    QR-->>RAG: query (giữ nguyên hoặc đã viết lại)

    Note over RAG: Bước 3: Embedding câu hỏi
    RAG->>EMB: embed(query)
    EMB-->>RAG: query_vector [768 chiều]

    Note over RAG,KB: Bước 4: Hybrid Search (song song)
    RAG->>KB: searchSemantic(vector, storeId) + searchKeyword(query, storeId)
    Note over KB: Luồng 1: pgvector cosine distance<br/>Luồng 2: tsvector full-text search<br/>Cả hai lọc: store_id + is_in_stock = TRUE
    KB-->>RAG: Danh sách semantic + keyword results

    Note over RAG: Bước 5: RRF Fusion
    RAG->>RAG: score(d) = SUM(1/(60+rank)) → Top 5

    Note over RAG,CP: Bước 6: Co-purchase Enrichment
    RAG->>CP: getCoPurchaseProducts(topProductIds, storeId)
    CP-->>RAG: Sản phẩm thường mua kèm

    Note over RAG,LLM: Bước 7: Personalized Generation
...
Kiểm tra xem câu hỏi có chứa đại từ mơ hồ ("nó", "cái đó", "loại này") không. Nếu có và lịch sử chat tồn tại, gọi Phi-3 viết lại thành câu độc lập. VD: "Nó giá bao nhiêu?" → "Bia Tiger 330ml giá bao nhiêu?"

**Bước 3 — Query Embedding:**
Câu hỏi (đã viết lại nếu cần) được chuyển thành vector 768 chiều bằng Vietnamese SBERT. Mô hình chạy **local trên CPU** thông qua ONNX Runtime (`@xenova/transformers`).

**Bước 4 — Hybrid Search (song song):**
Chạy đồng thời 2 luồng tìm kiếm trên PostgreSQL:
...
Sau khi có Top-5, truy vấn `co_purchase_stats` để tìm sản phẩm thường mua kèm. VD: Bia Tiger → "Đá viên" + "Khô bò".

**Bước 7 — Personalized Augmented Generation:**
Dữ liệu Top-5 + co-purchase + thông tin khách hàng (customer_type, total_spent) được format thành context, ghép vào prompt chuẩn Phi-3. LLM sinh câu trả lời tự nhiên, cá nhân hóa theo từng khách.
